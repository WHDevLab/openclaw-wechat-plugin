...
export class WechatAccessWebSocketClient {
  ...
  sendMessageChunk = (sessionId: string, promptId: string, content: ContentBlock, guid?: string, userId?: string): void => {
    console.log(`${this.logPrefix} [sendMessageChunk] sessionId=${sessionId}, promptId=${promptId}`);
  ...
  sendToolCall = (sessionId: string, promptId: string, toolCall: ToolCall, guid?: string, userId?: string): void => {
    console.log(`${this.logPrefix} [sendToolCall] sessionId=${sessionId}, promptId=${promptId}`);
    const payload: UpdatePayload = {
      session_id: sessionId,
      prompt_id: promptId,
  ...
  sendToolCallUpdate = (sessionId: string, promptId: string, toolCall: ToolCall, guid?: string, userId?: string): void => {
    console.log(`${this.logPrefix} [sendToolCallUpdate] sessionId=${sessionId}, promptId=${promptId}`);
    const payload: UpdatePayload = {
      session_id: sessionId,
      prompt_id: promptId,
  ...
  sendPromptResponse = (payload: PromptResponsePayload, guid?: string, userId?: string): void => {
    ...
    console.log(`${this.logPrefix} [sendPromptResponse] sessionId=${payload.session_id}, promptId=${payload.prompt_id}, stopReason=${payload.stop_reason}, hasGuid=${!!guid}, hasUserId=${!!userId}, contentLen=${contentPreview.length}`);
  ...
  private connect = (): void => {
    ...
    console.error(`${this.logPrefix} 连接配置: hasToken=${!!this.config.token}, hasGuid=${!!this.config.guid}, hasUserId=${!!this.config.userId}`);
    const wsUrl = this.buildConnectionUrl();
    console.error(`${this.logPrefix} 正在连接 WebSocket...`);
  ...
  private handleRawMessage = (data: WebSocket.RawData): void => {
    try {
      ...
      const rawPreview = typeof data === "string" ? data.substring(0, 200) : data.toString().substring(0, 200);
      console.error(`${this.logPrefix} 消息解析失败:`, error, `原始数据(前200字符): ${rawPreview}`);
  ...
  private sendEnvelope = <T>(method: AGPMethod, payload: T, guid?: string, userId?: string): void => {
    ...
    try {
      ...
      this.ws.send(data);
      console.log(`${this.logPrefix} 发送消息: method=${method}, msg_id=${envelope.msg_id}, len=${data.length}`);
    } catch (error) {
